...
from karma_src.utils.coloring import COLOR, COLORED_COMBOS
from karma_src.utils.exceptions import karmaException, HostHandlerException
from karma_src.utils.request_handler import RequestHandler
from karma_src.utils.logger import SystemOutLogger
from karma_src.utils.help_utils import HelpUtilities
from karma_src.lib.fuzzer import URLFuzzer
from karma_src.lib.host import Host
from karma_src.lib.scanner import Scanner, NmapScan, NmapVulnersScan, VulnersScanner
from karma_src.lib.sub_domain import SubDomainEnumerator
from karma_src.lib.dns_handler import DNSHandler
from karma_src.lib.waf import WAF
from karma_src.lib.tls import TLSHandler
from karma_src.lib.web_app import WebApplicationScanner
from Vulnerability_scanner.Vuln_scanner import Vuln_scanner
...
@click.option("--vulners-path", default=os.path.join(MY_PATH, "utils/misc/vulners.nse"),
              help="Path to the custom nmap_vulners.nse script."
                   "If not used, karma uses the built-in script it ships with.")
...
@click.option("-o", "--outdir", default="karma_scan_results",
...
def main(target,
    ...
    #                 COLORED_COMBOS.NOTIFY))
    #         except karmaException as err:
    #             print("{}{}{}".format(COLOR.RED, str(err), COLOR.RESET))
    #             exit(3)

    #     main_loop = asyncio.get_event_loop()

    #     logger.info("{}### karma Scan Started ###{}\n".format(
