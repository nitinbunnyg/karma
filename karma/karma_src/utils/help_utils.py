...
from karma_src.utils.exceptions import karmaException, ScannerException, RequestHandlerException
...
class HelpUtilities:
    ...
    def validate_target_is_up(cls, host):
        ...
        try:
            ...
            try:
                ...
                raise karmaException("Target {} seems to be down (no response to ping or from a web server"
    ...
    def parse_cookie_arg(cls, cookie_arg):
        try:
            ...
            raise karmaException("Cookie parsing error occurred, probably due to invalid cookie format.\n"
    ...
    def validate_proxy_args(cls, *args):
        ...
        if not supplied_proxies:
            ...
            raise karmaException("Must specify only one of the following:\n"
    ...
    def validate_executables(cls):
        if not (cls.find_nmap_executable() and cls.find_openssl_executable()):
            raise karmaException("Could not find Nmap or OpenSSL "
                                 "installed. Please install them and run karma again.")
        if system() == "Darwin":
            if not cls.find_mac_gtimeout_executable():
                raise karmaException("To support karma with macOS 'gtimeout' must be installed.\n"
    ...
    def query_dns_dumpster(cls, host):
        ...
        try:
            ...
            for c in jar:
                ...
                break
            dnsdumpster_session.post(url, data=payload, headers={
