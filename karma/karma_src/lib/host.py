...
from karma_src.utils.exceptions import HostHandlerException
from karma_src.utils.help_utils import HelpUtilities
from karma_src.utils.coloring import COLOR, COLORED_COMBOS
from karma_src.utils.logger import Logger, SystemOutLogger
...
class Host:
    ...
    def _extract_port(self, addr):
        try:
            ...
            self.logger.info(
                "{} Did not detect port. Using default port 80".format(COLORED_COMBOS.NOTIFY))
            return
        return
        ...
        if "://" in domain:
            if any(domain.startswith(proto) for proto in ("https", "http")):
                ...
                raise HostHandlerException(
                    "Unknown or unsupported protocol: {}".format(self.target.split("://")[0]))
        return
    ...
    def create_host_dir_and_set_file_logger(self):
        log_file = HelpUtilities.get_output_path(
            "{}/dns_records.txt".format(self.target))
