...
from karma_src.utils.exceptions import karmaException
...
class SubDomainEnumerator:
    ...
    def __init__(self,
        ...
        log_file = HelpUtilities.get_output_path(
            "{}/subdomains.txt".format(self.target))
    ...
    def _extract_from_dns_dumpster(self):
        self.logger.info("{} Trying to extract subdomains from DNS dumpster".format(
        ...
        try:
            ...
            for row in hosts_table.find_all("tr"):
                tds = row.select("td")
                # Grab just the URL, truncate other information
                sub_domain = tds[0].text.split('\n')[0]
                self.logger.info("{} Found subdomain in DNS dumpster: {}".format(
                    COLORED_COMBOS.GOOD, sub_domain))
        except (karmaException, IndexError):
            self.logger.info(
                "{} Failed to query DNS dumpster for subdomains".format(COLORED_COMBOS.BAD))
    ...
    async def bruteforce(self):
        ...
        self.logger.info(
            "{} Bruteforcing subdomains".format(COLORED_COMBOS.NOTIFY))
