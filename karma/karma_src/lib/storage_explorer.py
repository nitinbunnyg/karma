...
from karma_src.utils.exceptions import karmaException, RequestHandlerException
from karma_src.utils.coloring import COLORED_COMBOS, COLOR
...
class AmazonS3Handler(Storage):
    ...
    def _is_s3_url(self, src):
        ...
        return any(("s3" in src and "amazonaws" in src,
                    ...
                    "cdn.{}".format(
                        ".".join(self.host.target.split(".")[1:])) in src,
    ...
    def _scan_for_sensitive_files(self, contents, url):
        ...
        for el in xpars.get("ListBucketResult").get("Contents"):
            ...
            for file in self.sensitive_files:
                if file in key:
                    self.logger.debug(
                        "Found {} file in bucket {}".format(key, url))
...
class StorageExplorer(AmazonS3Handler, GoogleStorageHandler, AzureStorageHandler):
    ...
    def run(self, soup):
        ...
        if self.s3_buckets:
            self.logger.info("{} S3 buckets discovered. Testing for permissions".format(
            ...
            if self.num_files_found > 0:
                ...
                self.logger.info("{} No sensitive files found in target's cloud storage".format(
                    COLORED_COMBOS.BAD))
            else:
