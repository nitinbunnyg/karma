...
from karma_src.utils.web_server_validator import WebServerValidator
from karma_src.lib.storage_explorer import StorageExplorer
...
from karma_src.utils.exceptions import WebAppScannerException, WebServerValidatorException
...
class WebApplicationScanner:
    ...
    def __init__(self, host):
        ...
        log_file = HelpUtilities.get_output_path(
            "{}/web_scan.txt".format(self.host.target))
    ...
    def _xss_protection(self):
        ...
        if xss_header and "1" in xss_header:
            self.logger.info(
                "{} Found X-XSS-PROTECTION header".format(COLORED_COMBOS.BAD))

    def _cors_wildcard(self):
        if self.headers.get("Access-Control-Allow-Origin") == "*":
            self.logger.info(
                "{} CORS wildcard detected".format(COLORED_COMBOS.GOOD))
    ...
    def _sitemap(self):
        ...
        if res.status_code != 404 and res.text and "<!DOCTYPE html>" not in res.text:
            self.logger.info(
                "{} Found sitemap.xml".format(COLORED_COMBOS.GOOD))
