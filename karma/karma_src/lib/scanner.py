...
from karma_src.utils.logger import Logger
...
class NmapScan:
    ...
    Start karma with sudo for -sS else will run -sT
    """
    ...
    def __init__(self, host, port_range, full_scan=None, scripts=None, services=None):
        ...
        self.path = HelpUtilities.get_output_path(
            "{}/nmap_scan.txt".format(self.target))
        self.logger = Logger(self.path)

    def build_script(self):
        script = ["nmap", "-sV", "-oX", self.target+'.xml', self.target]
        ...
        if self.full_scan:
            ...
            script.append("-sC")
            self.logger.info(
                "{} Added scripts and services to Nmap script".format(COLORED_COMBOS.NOTIFY))
            return script
        else:
            if self.scripts:
                self.logger.info(
                    "{} Added safe-scripts scan to Nmap script".format(COLORED_COMBOS.NOTIFY))
                script.append("-sC")
            if self.services:
                self.logger.info(
...
class NmapVulnersScan(NmapScan):
    ...
    def __init__(self, host, port_range, vulners_path):
        ...
        self.path = HelpUtilities.get_output_path(
            "{}/nmap_vulners_scan.txt".format(self.target))
        self.logger = Logger(self.path)

    def build_script(self):
        script = ["nmap", "-Pn", "-sV", "--script",
                  self.vulners_path, self.target]
...
class Scanner:
    ...
    def run(cls, scan):
        ...
        scan.logger.info("{} Nmap script to run: {}".format(
            COLORED_COMBOS.INFO, " ".join(script)))
    ...
    def _parse_scan_output(cls, result):
        ...
        for line in result.split("\n"):
            ...
            if "/tcp" in line or "/udp" in line and "open" in line:
                line = line.split()
                parsed_output += "\t{}{}{} {}\n".format(
                    COLOR.GREEN, line[0], COLOR.RESET, " ".join(line[1:]))
...
class VulnersScanner(Scanner):
    ...
    def _parse_scan_output(cls, result):
        ...
        out_versions = re.sub(r"(\d+\/(?:tcp|udp))",
                              COLOR.GREEN + r"\1" + COLOR.RESET, out_versions)
        out_versions = re.sub(r"(\sCVE\S*)", COLOR.RED +
                              r"\1" + COLOR.RESET, out_versions)
        out_pure = re.sub(r"(\d+\/(?:tcp|udp))", COLOR.GREEN +
                          r"\1" + COLOR.RESET, out_pure)
