...
class TLSCipherSuiteChecker:
    ...
    async def scan_ciphers(self, port):
        script = "nmap --script ssl-enum-ciphers -p {} {}".format(
            str(port), self.target).split()
...
class TLSHandler(TLSCipherSuiteChecker):
    ...
    def __init__(self, host, port):
        ...
        log_file = HelpUtilities.get_output_path(
            "{}/tls_report.txt".format(self.target))
    ...
    async def run(self):
        ...
        if self._tls_results_exist():
            self.logger.info(
                "{} Done collecting TLS data".format(COLORED_COMBOS.INFO))
